...
if [ "${PACKAGE}" == "all" ]; then
	...
	sudo SSH_AUTH_SOCK=$SSH_AUTH_SOCK SSH_AGENT_PID=$SSH_AGENT_PID rsync --exclude='*root/' --exclude='dev' --exclude='proc' --exclude='var/log' --exclude='mnt' --exclude='var/cache' --exclude='etc' -av ${SRC} ${DEST} || exit 1
fi
